...
噜噜在 **5 秒无操作**后会随机触发以下行为之一。正常模式与免打扰模式（禁止跑动）的概率分布不同：
...
开启后噜噜完全不走/跑动，其余行为概率重新分配：

| 行为 | 概率 | 持续时间 | 说明 |
|------|------|----------|------|
| 🫸 扭呼啦圈 | 15% | 5 秒 | 腰部左右扭动 + 呼啦圈旋转 |
| 😋 吃橘子 | 15% | 3 秒 | 橘子从头顶飞到嘴边，咀嚼后重新长出 |
| 🍊 顶橘子 | 15% | 2 秒 | 头顶橘子上下颠动 5 次 |
| 🛁 泡澡冥想 | 15% | 10 秒 | 浴缸泡澡，蒸汽泡泡不断升腾，双眼微闭 |
| 🥱 打哈欠睡觉 | 25% | 30 秒 | 先打哈欠，再入睡，睡觉时流口水逐渐变长 |
| 💕 放粉色爱心 | 15% | 2.5 秒 | 从胸前持续放出粉色爱心 |

> **免打扰模式**：右键菜单可勾选"禁止跑动（免打扰）"，开启后噜噜完全不走/跑动，仅在原地执行其他自动行为。
...
```bash
npm run build:mac
# 产物：dist/水豚噜噜-x64.zip, dist/水豚噜噜-arm64.zip
```
